...
def main():
    parser = argparse.ArgumentParser(
        description='Generate new parameter file from iteration file of a gfp_gaussian run')
    ...
    write_param_file(parameter_setting, parameter_init, args.outfile)
    return
...
# ================================================================================ #
def write_param_file(parameter_setting, parameter_init, outfile):
    with open(outfile, "w") as fin:
        fin.write("# Generated config file for simulated data\n")
        for k in parameter_setting.keys():

            step = str(max([float(parameter_init[k])/100. , 1e-9]))
